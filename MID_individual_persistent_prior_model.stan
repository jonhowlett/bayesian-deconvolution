...
parameters {
  ...
  real mu_init;  // group mean of persistent prior parameter

  real mu_beta; // group mean of individual slopes

  real<lower=0> sigma_init; // group standard deviation of persistent prior parameter

  real<lower=0> sigma_beta; // standard deviation for group distribution of slopes

  vector[N] init; // individual persistent prior parameters
...
model {
  ...
  sigma_init ~ normal(0, 10);

  init ~ normal(mu_init, sigma_init);
  ...
  for(n in 1:N){
    ...
    for (t in 1:T) {
      ...
      adapt = 1.0/3.0 * (adapt + C_abs[n,t] + init[n]);
